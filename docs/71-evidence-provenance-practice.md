...
| 2 | ~~没有任何东西把一句主张连到一段字节~~ **已交付** | 逐字引文核对闸门；**没有**上蕴含判断 | **INV-660** ✅ |
| 3 | ~~被裁掉的东西 agent 拿不回来~~ **已交付** | 宿主中介、只读的按调用 id 回读（`ReadKept`） | **INV-661** ✅ |
| 4 | ~~摘要不指回它替换的区间；证据指针上限 10 条~~ **已交付** | OpenHands 的 `forgotten_event_ids` 形状，外加给复合误差设界 | **INV-662** ✅ |
...
### 7.2 INV-660 交付记录（2026-09-23）：阈值是量出来的，而量出来的最重要的东西是一条限制

阈值用 `fixtures/read-shapes` 的三个真实页面标定，正负两侧都测：

| 样本 | 结果 |
|---|---|
| 真实近逐字引文（丢词、改词、重排、换标点、掐头去尾） | 在 0.55 处召回 100% |
| 取自**另一个页面**的句子（152 条） | 最高分 0.329 |

正负之间的空隙很宽，阈值落在中间。第一版我**选**了 0.82 而不是量，那会丢掉八分之一的真引文。

**然后是那条限制，它比能力重要得多。** 拿一个真实句子做对照：

| 引文 | 分数 |
|---|---|
| 逐字 | 1.000 |
| 把 `is effective` 改成 `is not effective` | 0.979 |
| 把 `improves` 改成 `does not improve` | 0.949 |
| 把 `every benchmark` 改成 `no benchmark` | 0.973 |

**一句把原文意思完全反过来的引文，得分 0.95 以上。** 编辑距离是相对长度的，反转一个两百字句子
只需要四个字符，它看不见。而且没有任何阈值能分开：高到足以拒绝上面这些的阈值，会把每一条真实的
近逐字引文一起拒掉。

所以交付的判据是**精确匹配是唯一强信号**，近似匹配只是一个指针：它回答「这份来源里最接近的一段
在这里，它实际上是这么写的」，并把**来源的原话一起返回**，让两段话能并排看。近似匹配永远不计入
「已核实」。这条限制钉成了测试（`quote-check.test.ts` 最后一条），下一个想「把阈值调高到可以信任
近似匹配」的人会先撞上它。

顺带一条工程记录：直引号 `"` 开合同形，正则配不出来。`He said "yes" and then "yes" again` 交给正则
得到的是 ` and then `——两段引文**之间**的文字。只能顺序配对，这不是可以猜的东西。

### 7.3 INV-662 交付记录（2026-09-23）

`SummaryEntry` 多了 `replaced`：`{ from, to, turnIds, generation }`，并在摘要正文**之前**写一行
可被同一个正则读回的引用：`[summarises entries 0-340, turns … ; generation 2]`。放在前面是因为
摘要正文有机械字数上限，会被裁；引用必须活得比裁剪久。失败的那条路也记——「这些条目被丢了」是
很差的记录，「第 0 到 340 条、属于这几个 turn 的条目被丢了」才是能用的。

**证据指针的上限去掉了。** `CATEGORY_CAPS` 的本意是别让十六进制噪声挤掉真实路径，而指向「被裁掉
的那整份东西」的指针恰恰是这个窗口里最贵的东西。一个 turn 读了 15 个页面，原先只有 10 个指针进
摘要，另外 5 个无声消失。

**给复合误差设了界，这是调研里明确指出「没有人做」的一条。** 反复压缩会对上一份摘要再做摘要，
Anthropic 的按需压缩文档直说它就是这么干的，而没有任何厂商或论文给它设过上限。现在每一段历史带
`generation`，到 `MAX_SUMMARY_GENERATIONS`（4）之后那份摘要被**原样带过去**而不再被转述一遍。
这个数字是判断不是测量，写在代码注释里以便被反驳。

### 7.4 INV-665 交付记录（2026-09-23）：把两条已交付工作都卡住的那条边补上

`turns.jsonl` 的 `end` 记录多了 `evidence`：这一轮产生的留存指针，按写下的样子。**不新开账本**——
INV-634 之后 `turns.jsonl` 已经是 `record`，归档而不清空，正好是这条边该待的地方。

指针是从**落盘后的结果文本**里读出来的，不是从工具里一路传出来的：留存页和留存结果两种指针在
INV-659 之后已经是同一个形状、同一个解析器，都在 `storableResult` 交回的文本里。一个地方收集，
没有任何工具需要知道这条边存在。

两处用上了它：
- **引文闸门接进交付路径**。一次回答里的引号内容，在人看到之前拿这一轮真正读过的正文核一遍。
  每个 turn 最多一次，全部精确时**什么都不说**。措辞永远不是指控：找不到的引文可能是转述、
  可能是翻译、可能引自更早一轮读过的东西。
- **保留期跟着引用走**。`referencedKeptPaths()` 读活动账本，被某个活着的 turn 指着的文件过期也
  不删。只读活动文件：一个老到已被归档的 turn，比它指向的任何东西的保留期都老，扫几个月的归档
  是花很多力气保护不了任何东西。

实现里撞到的一件事值得记：给 `pruneFetched` 加 `referenced` 参数那次编辑**根本没落盘**，而
`npx tsc` 是绿的——因为调用处用的是条件展开 `...(referenced !== undefined ? { referenced } : {})`，
**对象字面量的多余属性检查被展开抑制了**。是测试而不是类型检查抓到的。展开一个可选字段很方便，
代价是它同时关掉了那个字段存不存在的检查。
