...
1. ~~**首次运行仍是 CLI 与 web 的混合**~~ **更正 + 已补（INV-542）**：主线其实一直在——
   `refreshSetup()` 的四步卡（箱子 → 第一个 agent → 门 → 第一件活），做完自动消失，`?` 按钮召回。
   我上一版把它写成"没有主线"是**错的**，写之前没去翻实现。真正缺的是两件小事，已经补上：
   每一步旁边给出**等价的命令**（`agentbox box up` / `agent new` / `chat`），因为服务器上装的人
   没有窗口可点；以及几处空状态从"没有 X"改成"X 是什么、下一步按哪里"。
...
3. ~~**语言两张皮**~~ **机制已落地（INV-544），UI 分片迁移中**：一份 bundle
   （`src/i18n/messages.ts`，zh/en 扁平点号键）+ `tr()` + **key 与占位符对齐的测试**——少一条、
   或者两边填的空不一样，测试就红。语言选择按"谁更当真"排序：**本人设的 > 浏览器要的
   （Accept-Language 按权重）> 门暗示的（飞书/钉钉=zh、Telegram=en）> 安装默认 > zh**。
   `channels/strings.ts` 36 条**全部迁完**，每个函数多一个可选 locale、缺省仍是安装默认，
   所以现有调用点一个都不用改、钉住中文措辞的测试也全绿。web 通过 `GET /api/i18n` 取同一份，
   **第一片**（attention 面板、首次运行卡、boxes 空状态）已接上，`?lang=en` 当场切。
   **剩下的**：`app-html.ts` 还有几千行英文字面量，按屏迁移不一次性动——半迁移的界面比
   一门语言更难读。
