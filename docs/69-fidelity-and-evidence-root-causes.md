...
> **2026-09-23 补（INV-663，PR #217）。** 上面这段测量还有第二个后果，当时没看出来：既然
> 那页确实带着 82% 的正文，那么**两个第三方都不通时就不该说「拿不到」**。原先两条 API 都失败
> 时报 `unavailable`，而 `unavailable` 在读取契约里的意思是「没有东西可读」——用它来表示
> 「我们的两个供应商今天不通」是说错话，而这套契约的全部价值就在于这个词是准的。
> 现在两条 API 都失败会退回普通抓取，报 `clipped` 并说明缺了什么（线程、作者、日期，正文在
> 登录提示之后），留存的 frontmatter 记 `fetcher: web-fetch`。墓碑（deleted / suspended /
> private）仍然是 `unavailable`，而且根本不走这条退路——它是正常返回的，带着原因。
